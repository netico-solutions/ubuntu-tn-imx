...
install_firmware()
{
	...
	# Copy the DDR and HDMI firmware into U-boot src directory
	UBOOT_DIR=${TWD}/output/u-boot/u-boot-tn-imx
	if [ -d $UBOOT_DIR ] ; then
		cp ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/lpddr4_pmu_train_1d_dmem_202006.bin ${UBOOT_DIR}/lpddr4_pmu_train_1d_dmem.bin
		cp ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/lpddr4_pmu_train_1d_imem_202006.bin ${UBOOT_DIR}/lpddr4_pmu_train_1d_imem.bin
		cp ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/lpddr4_pmu_train_2d_dmem_202006.bin ${UBOOT_DIR}/lpddr4_pmu_train_2d_dmem.bin
		cp ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/lpddr4_pmu_train_2d_imem_202006.bin ${UBOOT_DIR}/lpddr4_pmu_train_2d_imem.bin
		cp ${TWD}/${MKIMAGE_DIR}/${SOC_DIR}/signed_hdmi_imx8m.bin ${UBOOT_DIR}
	fi
...
{
	echo "*************************************************************"
	echo "Before running this script, please build u-boot first!
	"
